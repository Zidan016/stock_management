...
class Stockoutv extends StatefulWidget {
  const Stockoutv({super.key});

  @override
  State<Stockoutv> createState() => _StockoutvState();
}

class _StockoutvState extends State<Stockoutv> {
              ...
              actions: [
                TextButton(
                  onPressed: () async {
                    ...
                    int newStok = int.parse(newStock.text);
                    if(newStok>stok){
                      setState((){
                        alert = 'Your stock is not enough!';
                      });
                    }
                    else {
  ...
  stockOut()async{
    final respone = await repStok.stockOut(selected);
    if(respone == true){
      await getAllData();
      ceklist = 0;
      selected = [];
